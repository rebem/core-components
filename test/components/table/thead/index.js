...
describe('table/thead', () => {
    describe('basic', () => {
        it('exists', () => {
            expect(TableThead).to.exist;
        });

        it('is a component', () => {
            expect(TestUtils.isCompositeComponent(renderOnce(TableThead(), 'table'))).to.be.true;
        });
    });
    ...
    describe('render', () => {
        beforeEach(function() {
            this.renderWithProps = props => {
                this.rootComponent = renderOnce(TableThead(props), 'table');
